...
static void connection_on_timeout(verto_ctx *ctx, verto_ev *ev)
{
    ...
    if (connection->state_machine->state == LDAP_CONNECTION_STATE_RUN)
    {
        ...
        enum OperationReturnCode rc = ld_rename(connection, "cn=adam,ou=users,dc=domain,dc=alt", "cn=adam2", "ou=users,dc=domain,dc=alt", false);
...
static enum OperationReturnCode connection_on_error(int rc, void* unused_a, void* connection)
{
    ...
    fail_test("Entry rename was not successful\n");

    return RETURN_CODE_SUCCESS;
}

Ensure(Cgreen, entry_rename_test)
{
...
int main(int argc, char **argv) {
    ...
    add_test_with_context(suite, Cgreen, entry_rename_test);
