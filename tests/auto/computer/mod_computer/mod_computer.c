...
static LDAPAttribute_t** fill_user_attributes(TALLOC_CTX* ctx)
{
    ...
    attrs[0]->name = talloc_strdup(ctx, "serialnumber");
    attrs[0]->values[0] = talloc_strdup(ctx, "1234");
...
static void connection_on_timeout(verto_ctx *ctx, verto_ev *ev)
{
    ...
    if (connection->state_machine->state == LDAP_CONNECTION_STATE_RUN)
    {
        ...
        enum OperationReturnCode rc = ld_mod_computer(connection->handle, "test_mod_computer", "ou=equipment,dc=domain,dc=alt",
