...
{
    {
        "Test existing user unblock in OpenLDAP",
        "test_block_user",
        RETURN_CODE_SUCCESS
    }
};
...
static void connection_on_timeout(verto_ctx *ctx, verto_ev *ev)
{
    ...
    if (connection->state_machine->state == LDAP_CONNECTION_STATE_RUN)
    {
        ...
        for (int test_index = 0; test_index < current_testcases.number_of_testcases; test_index++)
        {
            ...
            enum OperationReturnCode rc = ld_unblock_user(connection->handle, testcase.entry_cn, NULL);
...
Ensure(Cgreen, user_unblock_test)
{
...
int main(int argc, char **argv) {
    ...
    add_test_with_context(suite, Cgreen, user_unblock_test);
