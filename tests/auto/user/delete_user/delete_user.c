...
static void connection_on_timeout(verto_ctx *ctx, verto_ev *ev)
{
    ...
    if (connection->state_machine->state == LDAP_CONNECTION_STATE_RUN)
    {
        ...
        int rc = ld_del_user(connection->handle, "test_delete_user", "ou=users,dc=domain,dc=alt");
...
static enum OperationReturnCode connection_on_error(int rc, void* unused_a, void* connection)
{
    ...
    fail_test("User deletion was not successfull\n");

    return RETURN_CODE_SUCCESS;
}

Ensure(Cgreen, user_del_test)
{
...
int main(int argc, char **argv) {
    ...
    add_test_with_context(suite, Cgreen, user_del_test);
